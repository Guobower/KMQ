...
class StockPicking(models.Model):
    ...
    def _get_sale_order(self):
        ...
        for data in self:
            ...
            if data.origin:
                ...
                origin = False
            if origin:
                sale_order_id = sale_obj.search([('name','=',origin)],limit=1)
	...
	for data in self:
	        if data.state == 'assigned' and data.picking_type_id.name == 'Delivery Orders':
        	    source_doc = data.origin
	            if ':' in data.origin:
        	        source_doc = data.origin.split(':')[0]
	            inv_obj = self.env['account.invoice'].search([('partner_shipping_id','=',data.partner_id.id),('origin','=',source_doc)])
        	    if inv_obj.partner_id and inv_obj.partner_id.parent_id.account_type == 'cod' and inv_obj.state != 'paid':
	                data.state = 'waiting'
	return res
    ...
    def do_new_transfer(self):
        ...
        if source_doc:
            if not inv_obj and self.picking_type_code == 'outgoing':
                ...
                for inv in inv_obj:
                    if inv.partner_id and inv_obj.partner_id.parent_id.account_type == 'cod' and inv.state != 'paid':

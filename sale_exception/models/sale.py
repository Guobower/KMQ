...
class SaleOrder(models.Model):
    ...
    def check_sale_exception_constrains(self):
	    ...
	    if self.partner_id.account_type == 'account': #Jagadeesh addess if else
                exception_ids = self.detect_exceptions()
	    else:
		exception_ids = []
            if exception_ids:
	...
	#print 'sale exception action confirm================'
	if self.partner_id.account_type == 'account': #Jagadeesh added if
	...
	#print 'search excep==============',order_exceptions,line_exceptions
        all_exception_ids = []
        for order in self:
            ...
            exception_ids = order._detect_exceptions(order_exceptions,
                                                     line_exceptions)
	    #print 'got exception ids================',exception_ids
	...
	#print 'in _exception_rule_eval_context'
    ...
    def _detect_exceptions(self, order_exceptions,
        ...
        self.ensure_one()
        exception_ids = []
	#print 'args==============',order_exceptions,line_exceptions
        for rule in order_exceptions:
            if self._rule_eval(rule, 'order', self):
	        #print 'if rule===============orders'

...
class PayuController(http.Controller):
		...
		sale_order_id.action_cancel()
		    ...
		    result = inv_obj.action_invoice_open()
                    ...
                    if account_invoice[0]['state']!='paid':
                        journal_ids = request.env['account.journal'].sudo().search([('name','=ilike','Bank')],limit=1)
			...
			account_payment = {
			    ...
			    'amount':order_amount_total,#sale_order[0]['amount_total'],
			...
			project = request.env['project.project'].sudo().create({'name':str(inv_obj.partner_id.name)+':'+str(inv_obj.origin),'sale_order_id':sale_order_id,'invoice_id':invoice_id})
	...
	##Coupons
	'''order = request.env['sale.order'].sudo().search([('name','=',post['reference'])])
	print post,'postttttt'
	print order,'orderrrrr'
	post['amount'] = str(float(order.amount_untaxed)+(float(order.amount_untaxed)*0.1)+order.amount_tax)'''
	...
	#for deposit payments
        ...
        Order = request.env['sale.order'].sudo().search([('name','=',post['reference'])])
        order_amount_total = False
	#print 'ref=============',post['reference'],Order,Order.payment_type
	
        if Order.payment_type == 'deposit':
            order_amount_total = (Order.payment_deposit * Order.amount_total)/100
            #order_amount_total = (Order.payment_deposit * amount)/100
        else:
            order_amount_total = Order.amount_total
        amount = str(round(order_amount_total,2))
        #Jagadeesh end  
	#print 'amount============',amount
        ...
        transactionDetails['basket']['description'] = 'KMQ'
	...
	transactionDetails['additionalInformation'] = {}
        ...
        transactionDetails['additionalInformation']['returnUrl'] =  'http://kmquat.odoo.co.za/payment/payu_com/dpn'
        transactionDetails['additionalInformation']['cancelUrl'] =  'http://kmquat.odoo.co.za/payment/payu_com/cancel'
    ...
    def payuMeaSetTransactionApiCall(self,args):    
	    ...
	    plugin = MyPlugin()
            ...
            return "http://kmquat.odoo.co.za/shop/unsuccessful"
